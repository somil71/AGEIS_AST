# TEST_INFRA: Comprehensive 4-Tier E2E Test Infrastructure Specification

This document provides the authoritative technical specification and test matrix for NEEDLE-SENTINEL across all 20 features (F1 through F20).

---

## 1. Test Suite Architecture

- **Test Suite Location**: `tests/e2e_sentinel_tests.rs`
- **Fixtures Directory**: `tests/fixtures/`
- **Test Strategy**: Opaque-box CLI and API requirement-driven testing across 4 tiers (230 tests total).
- **Execution Modes**:
  1. Default Feature Mode: `cargo test --test e2e_sentinel_tests`
  2. Sovereign Mode: `cargo test --test e2e_sentinel_tests --no-default-features --features sovereign`

### Sandboxed Test Context (`SentinelTestContext`)
```rust
...
## 2. Fixture Directory Specification (`tests/fixtures/`)
...
├── keys/
│   ├── test_auditor_ed25519.priv     # 32-byte Ed25519 private key seed
│   ├── test_auditor_ed25519.pub      # 32-byte Ed25519 public key hex
│   ├── secondary_auditor.priv        # Secondary private key
│   ├── secondary_auditor.pub         # Secondary public key
│   └── corrupted_key.priv            # Truncated invalid key file
├── ledgers/
...
## 3. Tier 1: Feature Coverage (100 Tests: 5 Tests × 20 Features)
...
## 4. Tier 2: Boundary & Corner Cases (100 Tests: 5 Tests × 20 Features)
...
| **T2-F01-02** | F1 Sovereign Gating | `t2_f01_both_cloud_and_sovereign_conflict` | Mutually exclusive build check | `cargo check --features "cloud sovereign"` | Deterministic build behavior (either cleanly gates or priority resolves) |
...
| **T2-F14-02** | F14 Canonical Encoding | `t2_f14_canonical_empty_json_array` | Value `json!([])` | `canonical_json_bytes(&val)` | Returns `b"[]"` |
...
## 5. Tier 3: Cross-Feature Integration Matrix (20+ Tests)
